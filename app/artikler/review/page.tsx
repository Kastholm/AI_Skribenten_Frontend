...
export default function ReviewPage() {
  ...
  const handlePublish = async (articleId: number) => {
    ...
    try {
      ...
      if (response.ok) {
        // Remove from review list or update status
        setArticles(articles.filter((article) => article.id !== articleId))
      } else {
          ...
          <div className="flex flex-1 flex-col gap-6 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground">Articles pending review and approval</p>
              </div>
              <ViewToggle view={view} onViewChange={setView} />
            </div>

            {loading ? (
              <div>Loading review articles...</div>
            ) : articles.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">No articles pending review</div>
            ) : view === "card" ? (
